...
class DiamondEmailController extends Controller
{
    ...
    public function dlDropHintApi(Request $request)
    {
        ...
        if ($response->success == 'true' || $hintData->secret_key == null) {
            ...
            // $user['from'] = 'no-reply@gemfind.com';
            ...
            $emailData = [
                'subject' => 'Someone Wants To Drop You A Hint',
                'recipientEmail' => $request->email,
                'recipientName' => $request->name,
                // Add other data for the email as needed
            ];
            ...
            try {
                // Create an instance of the SendEmail Mailable class
                $sendEmail = new SendEmail($emailData);
    ...
    public function dlReqInfoApi(Request $request)
    {
        ...
        if ($response->success == 'true' || $reqData->secret_key == null) {
            ...
            // echo '<pre>';
            // print_r($shopData->original['name']);
            // exit;

            //Sender Email

            $from_email_address = $reqData->from_email_address ? $reqData->from_email_address : '';
            ...
            Mail::send('diamondReqInfoSender', $data, function ($messages) use ($user) {
                ...
                // $messages->replyTo('no-reply@gemfind.com'); // Set the "reply-to" address
            });
