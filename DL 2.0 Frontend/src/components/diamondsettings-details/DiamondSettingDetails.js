  ...
  const initpingback = async () => {
    try {
      ...
      const response1 = await fetch(
        `http://platform.jewelcloud.com/DiamondTracking.aspx?RetailerID=${window.initData.data[0].dealerid}&VendorID=${productDetails.retailerInfo.retailerID}&DInventoryID=${getCurrentProductId}&URL=${window.location.href}&DealerStockNo=${productDetails.vendorStockNo}&Carat=${productDetails.stoneCarat}&Cut=${productDetails.shape}&Color=${productDetails.color}&Clarity=${productDetails.clarity}&Depth=${productDetails.depth}&Polish=${productDetails.polish}&Symmetry=${productDetails.symmetry}&FltPrice=${productDetails.fltPrice}&SellingPrice=${productDetails.fltPrice}&Girdle=${productDetails.girdleThick}&Culet=${productDetails.culet}&Fluorescence=${productDetails.fluorescence}&Measurements=${productDetails.measurement}&Certificate=${productDetails.certificate}&CertificateNo=${productDetails.certificateNo}&TableMes=${productDetails.table}&CutGrade=${productDetails.cutGrade}&UsersIPAddress=${data.IPv4}
      `,
        {
          method: "POST",
        }
